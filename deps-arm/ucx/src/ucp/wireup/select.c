        ...
        ucp_wireup_select_info_t *select_info)
{
    ...
    ucp_unpacked_address_for_each(ae, address) {
        ...
        if (!(remote_dev_bitmap & UCS_BIT(ae->dev_index))) {
            ucs_trace("line %ld, addr[%d]: not in use, because on device[%d]", __LINE__,
            ...
            ucs_trace("addr[%d]: not in use, because on md[%d]", addr_index,
                      ae->md_index);
            ////printf("%s: %s: %d\n", __FILE__, __FUNCTION__, __LINE__);
            //printf("line %ld, addr[%d]: not in use, because on md[%d]\n", __LINE__, addr_index,
              //        ae->md_index);
            continue;
            ...
            ucs_trace("addr[%d] %s: no %s", addr_index,
                      ...
                      ucp_wireup_get_missing_flag_desc(ae->md_flags,
                                                       ...
                                                       ucp_wireup_md_flags));
            ////printf("%s: %s: %d\n", __FILE__, __FUNCTION__, __LINE__);
            // //printf("line %ld, addr[%d] %s: no %s\n", __LINE__, addr_index,
            //           ucp_find_tl_name_by_csum(context, ae->tl_name_csum),
            //           ucp_wireup_get_missing_flag_desc(ae->md_flags,
            //                                            criteria->remote_md_flags,
            //                                            ucp_wireup_md_flags));
            //printf("%ld vs. %ld, %s\n", ae->md_flags, criteria->remote_md_flags, criteria->title);
            continue;
        }
        ...
        if (!ucs_test_all_flags(ae->iface_attr.cap_flags, criteria->remote_iface_flags)) {
            ...
            //                                            criteria->remote_iface_flags,
            //                                            ucp_wireup_iface_flags));
            //printf("ae->iface_attr.cap_flags = %ld vs.  criteria->remote_iface_flags = %ld, %s\n",ae->iface_attr.cap_flags, criteria->remote_iface_flags, criteria->title);
    ...
    if (UCS_BITMAP_IS_ZERO_INPLACE(&addr_index_map)) {
         ...
         p += strlen(p);
         //printf("UCS_BITMAP_IS_ZERO_INPLACE so go out\n");
         goto out;
    }
    ...
    UCS_BITMAP_FOR_EACH_BIT(tl_bitmap, rsc_index) {
        ...
        if (!ucp_wireup_check_flags(resource, md_attr->cap.flags,
                                    ...
                                    local_md_flags, criteria->title,
            //printf("%d, local md and interface check failed while %s, %s, %s, addr[%d],  md_attr->cap.flags = %ld and local_md_flags = %ld\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name, addr_index, md_attr->cap.flags, local_md_flags);
            ...
            if(!ucp_wireup_check_flags(resource, iface_attr->cap.flags,
                                    local_iface_flags, criteria->title,
                                    ucp_wireup_iface_flags, p, endp - p)){
                                                
            //printf("%d, local md and interface check failed while %s, %s, %s, addr[%d], iface_attr->cap.flags = %ld and  local_iface_flags = %ld,\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name, addr_index, iface_attr->cap.flags,local_iface_flags);
            ...
            if(!ucp_wireup_check_keepalive(select_params, resource,
                                        iface_attr->cap.flags,
                                        criteria->local_iface_flags, criteria->title,
                                        ucp_wireup_iface_flags, p, endp - p)) {
                                                
            //printf("%d, local md and interface check failed while %s, %s, %s, iface_attr->cap.flags = %ld, criteria->local_iface_flags = %ld\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name, iface_attr->cap.flags , criteria->local_iface_flags );
            ...
            if(!ucp_wireup_check_flags(resource, iface_attr->cap.event_flags,
                                    criteria->local_event_flags, criteria->title,
                                    ucp_wireup_event_flags, p, endp - p)) {
                                                
           // printf("%d, local md and interface check failed while %s, %s, %s, iface_attr->cap.event_flags = %ld, criteria->local_event_flags = %ld\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name, iface_attr->cap.event_flags, criteria->local_event_flags);
            ...
            if(!ucp_wireup_check_amo_flags(resource, iface_attr->cap.atomic32.op_flags,
                                        criteria->local_atomic_flags.atomic32.op_flags,
                                        32, 0, criteria->title, p, endp - p)) {
                                                
           // printf("%d, local md and interface check failed while %s, %s, %s\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name);
            ...
            if(!ucp_wireup_check_amo_flags(resource, iface_attr->cap.atomic64.op_flags,
           ...
           // printf("%d, local md and interface check failed while %s, %s, %s\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name);
            ...
            if(!ucp_wireup_check_amo_flags(resource, iface_attr->cap.atomic32.fop_flags,
          ...
          //  printf("%d, local md and interface check failed while %s, %s, %s\n", __LINE__, criteria->title, resource->dev_name, resource->tl_name);
            ...
            if(!ucp_wireup_check_amo_flags(resource, iface_attr->cap.atomic64.fop_flags,
                                        criteria->local_atomic_flags.atomic64.fop_flags,
                                        64, 1, criteria->title, p, endp - p))
        {
           // printf("local md and interface check failed while %s, %s, %s\n", criteria->title, resource->dev_name, resource->tl_name);
   ...
   // printf("string p = %s\n", p);
        ...
        UCS_BITMAP_FOR_EACH_BIT(rsc_addr_index_map, addr_index) {
            ...
            // //printf(UCT_TL_RESOURCE_DESC_FMT "->addr[%u] : %s score %.2f priority %d\n",
            //           UCT_TL_RESOURCE_DESC_ARG(resource),
            //           addr_index, criteria->title, score, priority);
            is_reachable = 1;
        ...
        //printf("%s: %s: %d, is_reachiable = %d, found = %d, rsc_idx = %d, dev_name = %s\n", __FILE__, __FUNCTION__, __LINE__, is_reachable, found, rsc_index, resource->dev_name);
...
out:
    ...
    ucs_trace("ep %p: selected for %s: " UCT_TL_RESOURCE_DESC_FMT " md[%d]"
              ...
              sinfo.score);
    //
    /*printf("ep %p: selected for %s: " UCT_TL_RESOURCE_DESC_FMT " md[%d]"
              " -> '%s' address[%d],md[%d],rsc[%u] score %.2f\n",
              ep, criteria->title,
              UCT_TL_RESOURCE_DESC_ARG(&context->tl_rscs[sinfo.rsc_index].tl_rsc),
              context->tl_rscs[sinfo.rsc_index].md_index, ucp_ep_peer_name(ep),
                            ...
                            ucp_wireup_select_context_t *select_ctx)
{
    ...
    for (i = 0; i < ucs_static_array_size(rndv_modes); i++) {
        ...
        if (added_lanes /* There are selected lanes */ ||
            ...
            (context->config.ext.rndv_mode != UCP_RNDV_MODE_AUTO)) {
                //printf("There are selected lanes  or there are no selected lanes, but a user requested the exact RNDV scheme, so there is no other choice\n");
            break;
        }
    }
                        ...
                        ucp_wireup_select_context_t *select_ctx)
{
    ...
    if (select_ctx->num_lanes == 0) {
        ...
        printf("No transports selected to %s (features: 0x%"PRIx64")\n",
                  select_params->address->name,
                  ucp_ep_get_context_features(select_params->ep));
        return UCS_ERR_UNREACHABLE;
    }
    //printf("%s returns UCS_OK\n", __FUNCTION__);
                           ...
                           unsigned *addr_indices, ucp_ep_config_key_t *key)
{
    ...
    for (lane = 0; lane < key->num_lanes; ++lane) {
        ...
        //printf("%s:%s, dst_sys_dev: %d\n", __FILE__, __FUNCTION__, select_ctx->lane_descs[lane].dst_sys_dev);
